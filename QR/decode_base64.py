import base64

class Base64:

    def __init__(self, encoded_hash):
        self.encoded_hash = encoded_hash
        

    def decode_base64_hash(self, hash):
        try:
            # Decodifica el hash en Base64
            decoded_hash = base64.b64decode(hash)
            return decoded_hash.decode('utf-8')
        except Exception as e:
            return f"Error: {str(e)}"


    def decode_long_hash(self):
        split_hash = []
        correct_hash = []
        for i in range(len(self.encoded_hash)):
            split_hash.append(self.encoded_hash[0])
            self.encoded_hash = self.encoded_hash[1:]
            try:
                hash64 = ''.join(split_hash)
                print(f"{base64.b64decode(hash64)}")
                correct_hash.append(hash64)
                split_hash = []
            except Exception as e:
                print(f"Error: {str(e)}")
        return correct_hash
    

    def test(self):
        x = 'sup'
        y = bytes(x, 'utf-8')
        print(y)
        z = base64.b64encode(y)
        print(z)
        a = base64.b64decode(z)
        print(a)
    


if __name__ == "__main__":
    encoded_hash = ""
    base_64 = Base64(encoded_hash)
    print(base_64.test())
    '''
    print(base_64.decode_long_hash())
    for hash in base_64.decode_long_hash():
        print(base_64.decode_base64_hash(hash))
    '''